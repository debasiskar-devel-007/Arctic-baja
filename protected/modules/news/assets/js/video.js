$(function(){
        ...
        if($('#wrapper-News_isvideo').find('#News_isvideo').is(':checked')){
            $('#desktop_video').hide();
            $('#youtube_video').show();
        }

    
    $('#wrapper-News_isvideo').find('#News_isvideo').change(function(){
        if($(this).is(':checked')){
            $('#desktop_video').hide();
            $('#youtube_video').show();
        }else{
    ...
    $('#file_upload_video1').uploadify({
        'multi':false,
        'wmode': 'transparent',
        'fileExt':'*.flv',
        'fileSizeLimit': '1073741824', 
        'swf'      :asset_url+'/swf/uploadify.swf',
        'uploader' : base_url+'/news/admin/news/uploadify_processvid',
                ...
                if(response.data.items)
                    ...
                    $(".addbut").click(function(){
                       ...
                       // $('#y_vid_name').val(video_id);
                        //$('#show_thumb1').append('');
                        $('#show_thumb1').append('<div style=width:480px;height:320px><object width=480 height=320><param value=http://www.youtube-nocookie.com/v/'+video_id+'?version=3&hl=en_US name=movie><param value=true name=allowFullScreen><param value=always name=allowscriptaccess><embed width=480 height=320 wmode=opaque allowfullscreen=true allowscriptaccess=always type=application/x-shockwave-flash src=http://www.youtube-nocookie.com/v/'+video_id+'?version=3&hl=en_US></object><img class=closeImg2 src='+base_url+'/images/close.png title=Close Image  style=margin-bottom:180px; onclick=removeVideo(this)> <input type=hidden  name=Newsvid[name][]  value='+video_id+'></div><br /><br /><br />');

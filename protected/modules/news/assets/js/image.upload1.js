$(function(){
    ...
    $('#file_upload2').uploadify({
        ...
        'uploader' : base_url+'/news/admin/news/uploadify_process1',
        ...
        'onUploadSuccess' : function(file, data, response) {
            var arr = news_imag_size;

            $.facebox('Resizing Uploaded File...<img src="'+theme_url+'/images/ajax_loading.gif">');
            $.post(base_url+"/news/admin/news/resizeimage1/",{'filename':data,imgarr:arr},function(res){
                ...
                for(n in arr){
                    imghtm += '<div class="image-main-div">' +
                        '<img style="max-height:150px;max-width:150px; border:solid 1px #979797;" src="'+base_url+'/uploads/news_image/thumb/'+arr[n][0]+'X'+arr[n][1]+'/'+data+'?version='+t+'" name="'+data+'" folder="'+arr[n][0]+'X'+arr[n][1]+'">' +
                        '<div class="crop-img"><img src="'+asset_url+'/images/crop.png" onclick="crop_image(\''+data+'\','+arr[n][0]+','+arr[n][1]+')"></div>' +
                        '</div>';

                }

                $('#show_thumb2').find('#clr').before('<div class="imglist-box2">'+
                    '<img src="'+base_url+'/images/close.png" class="closediv" onclick="delImage(\''+data+'\',this)">' +
                    imghtm+
                    '<input type="hidden" name="NewsImg[image][]" value="'+data+'" />' +
...
function crop_image(img,width,height)
{
    ...
    $("#myModal-img").children('div.modal-body').html('<img img_val='+img+' id=cur_image src="'+base_url+"/uploads/news_image/"+img+'">');
...

{
    ...
    $.post(base_url+'/news/admin/news/resize_cropImage1', {'image':image,'x2':x2,'y2':y2,'x1':x1,'y1':y1,'w':w,'h':h,width:width,height:height}, function(res) {

        var d = new Date();
        var n = d.getTime();

        $('img[name="'+image+'"][folder="'+width+'X'+height+'"]').attr('src',base_url+"/uploads/news_image/thumb/"+width+"X"+height+"/"+res+"?version="+n);
...
function delImage(image,obj){
    ...
    $.post(base_url+"/news/admin/news/delimage",{'image':image},function(res){
        $(obj).parent().remove();
    });
}

$(function(){
    ...
    $('#file_upload1').uploadify({
        ...
        'uploader' : base_url+'/news/admin/news/uploadify_process',
        ...
        'onUploadSuccess' : function(file, data, response) {
            ...
            $.post(base_url+"/news/admin/news/resizeimage/",{'filename':data,width:95,height:95,foldername:'thumb'},function(res){
                $('#show_thumb').html('<div class="imglist" style="position:relative; width:130px; margin-bottom:10px;"><a href="javascript:void(0);"><img name="'+data+'" src="'+base_url+'/uploads/proimage/thumb/'+data+'" style="max-height:181px;max-width:130px; border:solid 1px #979797;" /></a><input type="button" class="crop_btn" onclick="crop_image1(\''+data+'\')" value="Crop"><input type="button" style="margin-left:5px;" class="crop_btn" onclick="del_up_image(this)" value="Delete"><input type="hidden" name="News[user_image]" value="'+data+'" /></div>');
...
function crop_process1(image,x2,y2,x1,y1,w,h)
{
    ...
    $.post(base_url+'/news/admin/news/resize_cropImage', {'image':image,'x2':x2,'y2':y2,'x1':x1,'y1':y1,'w':w,'h':h,width:95,height:95,foldername:'thumb'}, function(res) {
...
function delTempImage(image){
        $.post(base_url+"/news/admin/news/delimage",{'image':image,'path':'./uploads/proimage/temp/'},function(res){ });
    }
